...
# Custom Functions and Imports from Google Doc
###Imports
import re
import matplotlib.pyplot as plt
import spacy
...
axes[0].set_title("Name of plot")
axes[1].imshow(high_cloud, interpolation='bilinear')
axes[1].set_title("Name of plot: example:<High Ratings>")
[ax.axis('off') for ax in axes]
fig.tight_layout();
fig.suptitle('Word Clouds - Raw Reviews', fontsize=20, y=1.05);

##Word Clouds of Tokenized Text
##We will define a single string for each group using the joined tokens.
...
##Once our groups are defined, we will make a word cloud using similar code as above.
